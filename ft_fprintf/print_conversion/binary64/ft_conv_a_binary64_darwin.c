...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_conv_a_binary64_darwin_bonus.c                  :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: stanaka2 < stanaka2@student.42tokyo.jp>    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/21 03:25:56 by stanaka2          #+#    #+#             */
/*   Updated: 2025/06/30 14:30:47 by stanaka2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_fprintf.h"

static void		ft_print_a(t_fprintf_ctx *ctx, t_my_double_hex *my_double_hex);
				...
				t_fprintf_ctx *ctx, t_my_double_hex *my_double_hex)
{
	...
	ft_fprint_char(ctx, my_double_hex->hex_int + '0');
	if (!(ctx->precision == 0 && !ctx->hash_flag))
		ft_fprint_char(ctx, '.');
	...
	{
		if (ctx->conv == 'a')
			ft_fprint_char(ctx, "0123456789abcdef"[my_double_hex->hex_frac[i]]);
		else
			ft_fprint_char(ctx, "0123456789ABCDEF"[my_double_hex->hex_frac[i]]);
		i++;
		precision--;
