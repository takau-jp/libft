...
/*   Updated: 2025/06/30 14:35:20 by stanaka2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_fprintf.h"

static void		ft_print_x(t_fprintf_ctx *ctx, uintmax_t num);
static size_t	ft_get_digits_uintmax_hexa(uintmax_t num);
static void		ft_print_hexa(t_fprintf_ctx *ctx, uintmax_t num);
...
static void	ft_print_hexa(t_fprintf_ctx *ctx, uintmax_t num)
{
	...
	if (ctx->conv == 'x')
		ft_fprint_char(ctx, "0123456789abcdef"[num % 16]);
	else
		ft_fprint_char(ctx, "0123456789ABCDEF"[num % 16]);
}
