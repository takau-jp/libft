...
/*   Updated: 2025/06/30 16:37:20 by stanaka2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_fprintf.h"

static void		ft_print_e(t_fprintf_ctx *ctx, t_my_double *my_double);
static size_t	ft_get_length_exp_notation(t_fprintf_ctx *ctx,
					t_my_double *my_double, int decimal_exp);
static void		ft_print_exp_notation_non_negative_exp(t_fprintf_ctx *ctx,
					t_my_double *my_double, int decimal_exp);
static void		ft_print_exp_notation_negative_exp(t_fprintf_ctx *ctx,
					t_my_double *my_double, int decimal_exp);
...
static size_t	ft_get_length_exp_notation(
					t_fprintf_ctx *ctx, t_my_double *my_double, int decimal_exp)
{
	size_t	len;
				...
				t_fprintf_ctx *ctx, t_my_double *my_double, int decimal_exp)
{
	...
	while (i < PF_DBL_INT && my_double->decimal_int[i] == 0)
		i++;
	ft_fprint_char(ctx, my_double->decimal_int[i++] + '0');
	if (!(ctx->precision == 0 && !ctx->hash_flag))
		ft_fprint_char(ctx, '.');
	...
	{
		ft_fprint_char(ctx, my_double->decimal_frac[i++] + '0');
		precision--;
	}
	while (precision--)
